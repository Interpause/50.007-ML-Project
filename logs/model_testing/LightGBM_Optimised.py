...
# --- 2. Define LightGBM Model and Hyperparameter Grid ---
print("\nDefining LightGBM model and hyperparameter grid...")
# Base LightGBM model (parameters that won't be searched are set here)
base_lgb_model = LGBMClassifier(
    ...
    # If tuning n_estimators, be careful as it can interact with early stopping
    # n_estimators=1000 # Example fixed value
)

# Define hyperparameter grid for tuning
# Note: Using a smaller grid or fewer options can speed up the search
param_grid = {
    'n_estimators': [800, 1000], # Number of boosting rounds
    'max_depth': [5, 6],        # Maximum tree depth
    'learning_rate': [0.08, 0.1], # Boosting learning rate
    'subsample': [0.7, 0.8],     # Subsample ratio of the training instances
    'colsample_bytree': [0.7, 0.8] # Subsample ratio of features when constructing each tree
...
# LightGBM's sklearn wrapper directly supports 'f1_macro'
...
train_proba = best_lgb_model.predict_proba(X_train_full)[:, 1]
...
test_proba = best_lgb_model.predict_proba(X_test)[:, 1]
...
print("\nCreating submission file: lgbm_tuned_submission.csv")
submission_df = pd.DataFrame({
    'id': test_ids,
    'label': final_predictions
})
# Save to CSV without index
submission_df.to_csv('lgbm_tuned_submission_opt.csv', index=False)
print("Submission file 'lgbm_tuned_submission.csv' created successfully!")
...
print(f"\nFor comparison (Tuned LightGBM model):")
...
print("\nTuned LightGBM model execution completed.")
