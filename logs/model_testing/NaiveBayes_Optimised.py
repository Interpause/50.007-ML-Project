...
# --- 2. Define MultinomialNB Model and Hyperparameter Grid ---
print("\nDefining MultinomialNB model and hyperparameter grid...")
# Base MultinomialNB model
base_nb_model = MultinomialNB()
# Note: MultinomialNB primarily has 'alpha' as the key hyperparameter to tune.
# Alpha is the additive (Laplace/Lidstone) smoothing parameter.

# Define hyperparameter grid for tuning
# Test a range of alpha values, including common defaults and others
param_grid = {
    'alpha': [0.01, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0]
}
...
# MultinomialNB's sklearn wrapper directly supports 'f1_macro'
grid_search = GridSearchCV(
    estimator=base_nb_model,
...
print("\nBest MultinomialNB model selected.")
...
train_proba = best_nb_model.predict_proba(X_train_full)[:, 1]
...
test_proba = best_nb_model.predict_proba(X_test)[:, 1]
...
print("\nCreating submission file: nb_tuned_submission.csv")
submission_df = pd.DataFrame({
    'id': test_ids,
    'label': final_predictions
})
# Save to CSV without index
submission_df.to_csv('nb_tuned_submission.csv', index=False)
print("Submission file 'nb_tuned_submission.csv' created successfully!")
...
print(f"\nFor comparison (Tuned MultinomialNB model):")
...
print("\nTuned MultinomialNB model execution completed.")
