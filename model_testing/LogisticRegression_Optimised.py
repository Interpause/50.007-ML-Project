...
# Import the required linear model library
...
# --- 1. Load Data ---
print("Loading data...")
# Load training data (assuming TF-IDF features are already computed)
...
# --- 2. Define Logistic Regression Model and Hyperparameter Grid ---
print("\nDefining Logistic Regression model and hyperparameter grid...")
# Base Logistic Regression model (parameters that won't be searched are set here)
base_lr_model = LogisticRegression(
    random_state=42,      # For reproducibility
    max_iter=1000,        # Increase if convergence warnings occur
    solver='liblinear'    # Good solver for small datasets, supports L1 and L2
)

# Define hyperparameter grid for tuning
# Note: Adjust the grid based on computational resources and desired exploration
param_grid = {
...
cv_splitter = StratifiedKFold(n_splits=5, shuffle=True, random_state=42) # Standard 5-fold CV

# Initialize GridSearchCV
# LogisticRegression's sklearn wrapper directly supports 'f1_macro'
...
train_proba = best_lr_model.predict_proba(X_train_full)[:, 1]
...
test_proba = best_lr_model.predict_proba(X_test)[:, 1]
...
print("\nCreating submission file: lr_tuned_submission.csv")
submission_df = pd.DataFrame({
    'id': test_ids,
    'label': final_predictions
})
# Save to CSV without index
submission_df.to_csv('lr_tuned_submission.csv', index=False)
print("Submission file 'lr_tuned_submission.csv' created successfully!")
...
print(f"\nFor comparison (Tuned Logistic Regression model):")
...
print("\nTuned Logistic Regression model execution completed.")
